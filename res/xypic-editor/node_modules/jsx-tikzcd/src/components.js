...
function renderEdge(vnode, co = false) {
    ...
    let p = ({left: '^', right: "_", inside: '|-'})[labelPosition]
    let [w1, w2] = vnode.attributes.value != null
        && needWrapChars.some(c => vnode.attributes.value.includes(c))
        ? ['{', '}'] : ['', '']
    let valueArg = vnode.attributes.value != null ? `${p}${w1}${vnode.attributes.value}${w2}` : ''
 //   let args = ['', valueArg, ...(vnode.attributes.args || [])].filter(x => x != null).join(', ')
    ...
    let xydict = ({
                        ...
                        */
    let args = vnode.attributes.args != null ? vnode.attributes.args.join('') : ''
    args = args != '' ? `@{${vnode.attributes.args.join('')}}` : ''
    return `\\ar${vnode.attributes.xybend}${args}[${vnode.attributes.direction}]${valueArg}`
}
