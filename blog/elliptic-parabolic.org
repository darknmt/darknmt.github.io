...
manifold, but the situation is much simpler here since we can deferentiate directly an element
in \( \mathcal{S}^*(M) \).
...
\mathbb{R}^3,\im\theta \leq 0 \) except \(0 \).
#+END_exampl

#+BEGIN_remark
If \( \sigma = \lcm (\sigma_1,\dots,\sigma_n) \) is the \( \lcm \) of weights of variable
...
be formulated using Index theorey for Fredhom maps:
...
The difference \( \dim \ker (\beta,\gamma) -\dim\coker (\beta,\gamma) \) therefore can be
