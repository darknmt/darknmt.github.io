...
is of the form
\[
 P(F) = \sum c_{\alpha_1,\dots, \alpha_\nu} (x, F(x)) D^{\alpha_1} F^{a_1} \dots D^{\alpha_nu} F^{a_{\nu}}
\]
...
\[
\|f\|_{B^{s,p}} := \sum_{\|\gamma\| < s} \|D^\gamma\|_{L^p} +
\sup_{v, s- \frac{\sigma}{\sigma_j} < \|\gamma\| < s} \frac{\|\Delta^v_j D^\gamma
...
2. Besov spaces \( B^{s,p}(X) \) are reflexive Banach space with their dual spaces being \( B^{-s,p}(X) \).
...
\), and define
\[
\Gamma f:= \left( x\mapsto \Gamma(x,f(x)) \right).
\]
Then 
\[ 
...
\]
where \( C \) depends only on \( B \) and the support of \( F \), but not \( F \).
#+END_lemma
...
bounded in \( B \) and compactly supported in \( V \), we see that \( \|f^\alpha\|_p \leq
...
1. For eqref:eq:est-term-small, it is sufficient to have
   ...
   \leq \frac{n + \|\gamma\|}{qs} < \frac{n+r}{qs} \).
...
It is sufficient then to take \( \theta = r- \|\gamma\|  \). Now the RHS of
...
\[
 \frac{\|\Delta^v_j D^\gamma (PF) \|_p}{|v|^{(r-\|\gamma\|)\sigma_j/\sigma}} \leq C \left(
...
W^{2,p}(M\times[\alpha,\alpha+\epsilon]) \) then \( F \) and \( \frac{\partial F}{\partial
x^i} \) are in \( C(M\times[\alpha,\alpha+\epsilon]) \) by [[file:sobolev-riemannian.org][Sobolev imbeddings]]. It makes sense then to talk
about:
...
2. the pointwise condition \( F: M\times [\alpha,\alpha+\epsilon] \longrightarrow
   B\subset V \).
#+END_remark
...
\frac{1}{q} > (\frac{r}{2}+1)\frac{1}{p} \). Apply Theorem ref:thm:reg-parabolic, for all
...
W^{r,*}([\pi,\omega]) \), therefore by Theorem ref:thm:reg-parabolic, \(
...
\restr{F}{M\times\{\omega\}} \), both are of weight \( 0 \): Since for \( q,r \) in Step 1,
