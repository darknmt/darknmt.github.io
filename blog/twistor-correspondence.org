...
#+LATEX:\iffalse
This post is my reading note for cite:Eells.Salamon85_TwistorialConstructionHarmonic writen using a more standard
...
Gr}_2(\mathbb{R}^n) \) the space of oriented 2-planes in \(\mathbb{R}^n \). We can then equip \( \widetilde {\rm
...
** Twistor spaces and their natural complex structures
   ...
   which contradicts the fact that under Koszul--Malgrange cite:Koszul.Malgrange58_CertainesStructuresFibrees complex structure
...
branched immersion cite:Gulliver.etal73_TheoryBranchedImmersions. We can now state the correspondence in cite:Eells.Salamon85_TwistorialConstructionHarmonic.
...
** Review: A few local properties of (pseudo)holomorphic curves.

#+ATTR_LATEX: :options [Dependence on \( \infty \)-jet]
#+ATTR_HTML: :id thm:infty-jet :btit (Dependence on \( \infty \)-jet) 
#+BEGIN_theorem
...
For a proof of this Theorem ref:thm:infty-jet, see cite:McDuff.Salamon00_JholomorphicCurvesQuantum . The
...
#+ATTR_HTML: :id rem:quad-diff-surface
#+BEGIN_remark
#+LATEX: \label{rem:quad-diff-surface}
An application of the last formula is that the tension field is
\[
 \tau(f) = -\tr_h \nabla df = - 4 \nabla_{\partial_z}\nabla_{\partial_{\bar z}}f,
\]
hence
\[
\frac{\partial}{\partial \bar z} \left\langle \frac{\partial f}{\partial z},  \frac{\partial
f}{\partial z} \right\rangle_g = -\frac{1}{2} \left \langle \tau(f), f_*
\frac{\partial}{\partial z} \right\rangle_g  
\]
Therefore the Hopf's quadratic differential \( q(f,[h],h):= \left \langle \frac{\partial
f}{\partial z}, \frac{\partial
f}{\partial z} \right \rangle_g dz\otimes dz \) is holomorphic if and only if the tension
field \( \tau(f) \) is orthogonal to the tangent \( f_* T\Sigma \) of the surface. In
particular if \( f \) is harmonic then its quadratic differential is holomorphic.

A consequence is that all harmonic map from the 2-sphere \( \mathbb{S}^2 \) is
quadratic. This is because \( \mathbb{S}^2 \) carries no non-zero holomorphic quadratic differential.
#+END_remark
...
\nabla_{\frac{\partial}{\partial z}}\sigma\in \Lambda^{0,2} \). Since we knew from
