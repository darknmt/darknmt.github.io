...
complex six-manifold \( S \) such that minimal surfaces in \( M \) correspond to
holomorphic curves in \( S_- \). The space \( S \), called *twistor space* of \( M \) is a \( \mathbb{S}_2 \)-bundle over \( M
...
One sees that eqref:eq:smooth-twistor-1 is equivalent to \( f \) being harmonic because \(
