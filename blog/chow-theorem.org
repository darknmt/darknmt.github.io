...
We will prove in is this post a algebraisation result of Chow. The arguments follow the
...
class \( L \) corresponds to a zero set of a (analytic or algebraic) of the line bundle \(
L \). Therefore the case of codimension 1 follows from the computational miracles:
1. All analytic line bundle of \( \mathbb{P}^N(\mathbb{C}) \) are of the form \(
...
these sections locally as holomorphic functions, and not as meromorphic functions as often
...
\end{split}   
\end{equation*}
where \( C = \max_\alpha \left[\sup_{r
