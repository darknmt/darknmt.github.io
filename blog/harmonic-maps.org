...
#+BEGIN_remark
The energy density is non-negative at every point. And \( E(f) = 0 \) if and only if \(
...
#+BEGIN_exampl
The energy functional \( E(f) \) is \( I_\frac{\sigma_1}{2}(f) \). \(
...
2. The *tension field* of \( f \) is the vector field along \( f \) defined by
   \[
   ...
   \rangle\).
3. \( f: M \longrightarrow M' \) is called *harmonic* if \( \tau(f)=0 \), or equivalently if
...
\), one has \(\Delta f^\gamma=0 \) since \( f^\gamma \) is holomorphic and \(
...
The *Hodge Laplacian* is defined as a endomorphism of \( A^p(M,E) \) given by
\[
...
2. \( \Delta s = (\Delta s)_i dx^i \) where \( (\Delta s)_i \) is an \( m\times m \)
   ...
   \]
   where:
   - the indices \( i,h,k \) correspond to local coordinates of \( M \), 
...
3. With \( s\cdot s' \) denoting the pointwise inner product of \( A^1(M,E) \) and \(
   \langle \cdot,\cdot \rangle_E \) denoting the metric \( g' \) of \( E \), one has
   \begin{equation}
   \label{eq:laplace-Q}   
...
3. The tension field of \( f \) is \( \tau (f) = -\delta f_*\).
#+END_proposition
#+BEGIN_proof
1. One has
   \[
...
#+BEGIN_corollary
#+ATTR_HTML: :id cor:harmonic-map-form
\( f: M \longrightarrow M' \) is a harmonic map of compact Riemannian manifolds if and only if \(
...
The function \( f \) is called *totally geodesic* if \( \beta(f) = 0 \) identically on \( M \).
#+END_definition
...
Ricci curvature of \( M \) give the following result in case the curvature of \(
...
\mathcal{H}_P := \ker T_P\pi^\perp \) is isometric to \( T_{\pi(P)}B \) and is said to be
...
\( f \) of the fibre, i.e. for every basic vector field \( X \), one has \(
...
\( i,j,k,\dots \) for \( M \), \( \alpha,\beta,\gamma,\dots \) for \( M' \) and \( a,b,c,\dots
\) for \( M'' \).
...
#+BEGIN_remark
1. The Proposition says that SFF of composition follows chain rule: the SFF of \( f'\circ f \) is a 2-tensor on \( M \) valued in
   \( TM'' \) is obtained by pushing forward the SFF of \( f \) and pulling back the SFF
   of \( f' \) and then adding the two. In Cartan formalism:
   \[
    (f'\circ f)_{\alpha i j} = (f'_{\alpha k}(f)\ f_{ki})_j = f'_{\alpha k h} f_{hj}
   f_{ki} + f'_{\alpha k} f_{kij} = f^* \beta(f') + f'_*\beta(f)
   \]
2. It is not true in general that the composition of harmonic maps are harmonic. For example,
...
translated as \(\Gamma'^n_{\alpha\beta} \geq 0 \) as a symmetric form (\( 1\leq\alpha,\beta \leq n-1
\)). This can be seen by the geometric intepretation of the second fundamental form of the
...
where \( D \) is a sufficiently small ball of dimension being the codimension of \( M' \) in \(
...
#+END_theorem
#+BEGIN_proof
Let \( \iota \) be the isometry of \( T \) locally given by \( (y,d)\mapsto (y,-d) \) for \(
...
-\Delta \rho + C\rho \), then by [[file:elliptic-parabolic.org][Maximum principle]], one has \( \rho = 0 \).
...
\). It can be proved that this is equivalent to eqref:eq:dev-sig-unique and the symmetries \( \lambda_{\beta\gamma,\delta} =
