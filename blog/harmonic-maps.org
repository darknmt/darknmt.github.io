...
The *connection Laplacian* is defined as a endomorphism of \( A^p(M,E) \) given by
\[
...
1. We note by \( Q(s) \) the last term of eqref:eq:laplace-Q, then \( Q \) is a (2,0)
...
2. \( {\rm Riem} \leq 0 \) (resp. \( {\rm Riem} < 0 \)) if all sectional curvatures are
...
1. If \( f \) is harmonic and \( Q(f_*) \leq 0 \) then \( f \) is totally geodesic and \(
   e(f) \) is constant.
2. If \( {\rm Ric}(M) \geq 0 \) and \( {\rm Riem}(M')\leq 0 \) then \( f \) is harmonic if
...
- \( R'_{\alpha\beta\gamma\delta} f^\alpha_i f^\beta_j f^\gamma_k f^\delta_l
   g^{ik}g^{jl} \) is \( (f^* R')_{ijhk}g^{ik}h^{jl} \). In a normal coordinate at \( P \)
...
If \( {\rm Riem}(M') < 0 \), one sees that all \( f_* e_i, f_* e_j\) are colinearmmetric, i.e. the image of \( Tf \) is of one dimension, which leads to the conclusion,
...
2. Take \( M''=M \) in Proposition ref:prop:compo-submersion and \( f=s: M \longrightarrow M' \) a
...
* Nonlinear heat flow: Global equation and existence of harmonic maps.
** Statement of the main results.

We want to prove in the next part the existence of harmonic map between manifolds \( M \)
and \( M' \) by deforming any map \( f: M
\longrightarrow M' \) using the heat flow, meaning solving the PDE:
\begin{equation}
...
clearly the energy of \( f_t \) decreases and we hope, under conditions, to obtain
convergence of \( \{f_t\} \) to a critical point \( f \) of \( E \), this will prove
that any homotopy class of \( C^\infty(M,M') \) has at least harmonic function.

It is proved by Eells and Sampson cite:eells_harmonic_1964 that
#+ATTR_LATEX: :options [Eells-Sampson]
#+ATTR_HTML: :id thm:eells-sampson :btit (Eells-Sampson) 
#+BEGIN_theorem
#+LATEX: \label{thm:eells-sampson}
Let \( M \) and \( M' \) be compact Riemannian manifolds with \( {\rm Riem } (M') \leq 0 \) then there exists a harmonic map \( f: M \longrightarrow M'
...
#+ATTR_LATEX: :options [Hamilton]
#+ATTR_HTML: :id thm:hamilton-bndry-Dirichlet :btit (Hamilton) 
#+BEGIN_theorem
#+LATEX: \label{thm:hamilton-bndry-Dirichlet}
...
About the terminology, *relative homotopy class* means that we only deform \( f \) among
maps with the same value on \( \partial M \). The *convexity of \( \partial M' \)* means
...
where \( D \) is a sufficiently small of dimension being the codimension of \( M' \) in \(
V \), and we will equip \( T \) with the product metric of \( M'\times D \). 
...
** Global equation and Uniqueness of nonlinear heat equation.
#+ATTR_LATEX: :options [Global equation]
#+ATTR_HTML: :id thm:global-eq :btit (Global equation) 
#+BEGIN_theorem
#+LATEX: \label{thm:global-eq}
...
#+END_theorem
#+BEGIN_proof
Let \( \iota \) be the isometry of \( T \) given by \( (y,d)\mapsto (y,-d) \) for \(
(y,d)\in M'\times D \equiv T \) 
...
#+ATTR_LATEX: :options [Uniqueness of solution of nonlinear hear equation]
#+ATTR_HTML: :id thm:unique-nonlinear-heat :btit (Uniqueness of solution of nonlinear hear equation) 
#+BEGIN_theorem
#+LATEX: \label{thm:unique-nonlinear-heat}
Let \( f_1,f_2: M\times [\alpha,\omega] \longrightarrow M' \) be \( C^2 \)
functions satisfying the non-linear heat equation
\(
 \frac{d f_i}{d t} = \tau_{M'}(f_i) 
\), i.e.
\[
...
-\Delta + C\rho \), then by [[file:elliptic-parabolic.org][Maximum principle]], one has \( \rho = 0 \).
...
Let \( c \) be a point in the chart \( V_i \) and choose the normal coordinates of \( M'
\) at \( c \). Then for \( a,b\in M' \) near \( c \), one has, since \( \sigma(a,b) =
\sigma(b,a) \) and \( \sigma(a,b)=0 \) if \( b^\gamma = k a^\gamma \) (the Euclidean
straight line from \( a \) to \( b \) viewed on \( M' \) is a geodesic):
\[
...
\end{align*}
where for the last inequality, we use \( 2uv \leq u^2 + v^2 \) and the fact that \( |df_1|
\) and \( |df_2| \) are bounded on \( M \). The estimate eqref:eq:first-order-w can be
continued:
\[
...
#+ATTR_HTML: :id rem:hamilton-alg-rig
#+BEGIN_remark
#+LATEX: \label{rem:hamilton-alg-rig}
...
\]
which was justified by \( \sigma(a,b) = \sigma(b,a) \) and \( \sigma(a,a)=0
\). Algebraically these symmetries of \( \sigma \) are not sufficient to justify the
...
\sigma}{\partial f_1^\beta \partial f_2^\gamma} {f_1}^\beta_i {f_2}^\gamma_j  \) in \(
(\beta,\gamma) \) and not just \( \frac{\partial^2
...
* A few energy estimates.
** Estimate of density energies
We finish this part with a few straightforward computation concerning the *potential
energy* \( e(f_t) = \frac{1}{2}|\nabla f_t|^2 \) and the *kinetic energy* \( k(f_t) =
\frac{1}{2}|\frac{\partial f_t}{\partial t}| \) of a nonlinear heat flow \( f_t \)
satisfying eqref:eq:loc-heat-flow.

#+ATTR_LATEX: :options [Density of Potential energy]
#+ATTR_HTML: :id thm:den-pot :btit (Density of Potential energy) 
#+BEGIN_theorem
#+LATEX: \label{thm:den-pot}
...
\(
\frac{d e(f_t)}{dt}= - \langle df_t, \Delta df_t \rangle
...
since \( t \) can be fixed after the third "=" sign.
#+END_proof

#+ATTR_LATEX: :options [Density of Kinetic energy]
#+ATTR_HTML: :id thm:den-kin :btit (Density of Kinetic energy) 
#+BEGIN_theorem
#+LATEX: \label{thm:den-kin}
If \( f_t \) satisfies eqref:eq:loc-heat-flow then 
\[
...
= f_t\) for \( t\in I =[\alpha,\omega] \) and \( E = F^* TM' \) is a Riemannian-connected bundle on \(
I\times M \) with curvature form \( \Theta \), then
\begin{equation}
\label{eq:den-kin-1}
...
** Estimate of total energies

We will now work with the total energies, in particular the *total potential energy* \(
E(f_t) := \int_M e(f_t) \) and *total kinetic energy* \( K(f_t) := \int_M k(f_t) \). Since
tension field is the gradient of \( E \), one has:

#+ATTR_HTML: :id thm:energy-cons
#+BEGIN_theorem
#+LATEX: \label{thm:energy-cons}
...
#+ATTR_HTML: :id thm:int-den-kin 
#+BEGIN_theorem
#+LATEX: \label{thm:int-den-kin}
...
1. The total potential energy \( E(f_t) \) is \( \geq 0 \), decreasing and convex.
2. The total kinetic energy \( K(f_t) \) is \( \geq 0 \), decreasing and if \( \omega =
   +\infty \) then \( \lim_{t\to\infty} K(f_t) = 0 \).
In particular, \( \int_{M\times \{\tau\}} |\nabla f|^2 \) and \( \int_{M\times\{\tau\}} \left|
\frac{\partial f_t}{\partial t} \right|^2 \) is bounded above by a constant \( C>0 \)
...
#+ATTR_HTML: :id thm:int-den-pot 
#+BEGIN_theorem
#+LATEX: \label{thm:int-den-pot}
If \( f_t \) satisfies eqref:eq:loc-heat-flow and \( \Riem(M')\leq 0 \) and \( \Ric(M) \)
is bounded below then 
\[
\int_M |\beta(f_t)|^2 \leq C
\]
for all time \( t \) where the constant \( C \) only depends on the curvature of \( M, M'
\) and the initial total potential and kinetic energy, in particular, \( C \) does not depend on \( t \).
#+END_theorem
...
#+ATTR_LATEX: :options [Boundedness in \( W^{2,2}(M) \)]
#+ATTR_HTML: :id cor:bound-2-2 :btit (Boundedness in \( W^{2,2}(M) \)) 
#+BEGIN_corollary
#+LATEX: \label{cor:bound-2-2}
