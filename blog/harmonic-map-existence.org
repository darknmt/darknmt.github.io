...
\]
We have proved that the solution [[file:polynomial-besov.org][exists in short-time and is
...
estimates will also be useful to justify that the solution \( F_t \) converges in \(
C^\infty(M) \) to a smooth function \( F_\infty \) which will eventually be a harmonic map
from \( M \) to \( M' \).

Recall that we [[file:harmonic-maps.org][proved]] in Corollary ref:cor:bound-2-2 the boundedness of \(
...
There exist a constant \( C \) depending only on \( \delta \), the curvatures and initial
...
The estimates of higher derivatives of \( F \) will be established in the following order: first in \(
W^{2,p} \) for all \( p \) norm then in \( W^{k,p} \) for all \( k,p \), then in \( C^\infty \).
...
For all \( p \in (1,+\infty) \), there exists a constant \( C>0 \) depending only on \(
...
\frac{\delta}{3} \). For the first term:
\[
...
Now apply \( L^1 \)-[[file:elliptic-parabolic.org][Comparison Theorem]] ref:thm:1-comparison to \( \psi_\tau
\), one has
\begin{equation}
\label{eq:lem:bound-2-p:3}
\|\psi_\tau\|_{L^1([\tau-
\frac{\delta}{2},\tau+\delta])} \leq \frac{3\delta}{2} \left\| \restr{\psi_\tau}{\tau-\frac{\delta}{2}}\right\|_{L^1(M)} =  \frac{3\delta}{2} \left\| \restr{e}{\tau-\frac{\delta}{2}}\right\|_{L^1(M)}\leq C
\end{equation}
where the constant is the initial potential energy.
...
\]
where, if we choose \( k \) sufficiently large, \( C_{\rm Sobolev} \) is the constant off Sobolev imbedding \(
...
\) is uniformly bounded on \( [\alpha +\delta,\omega) \). This proves that \( F \) extends
...
#+ATTR_HTML: :id thm:final
#+BEGIN_theorem
#+LATEX: \label{thm:final}
1. Let \( M, M' \) be compact Riemannian manifolds with \( \Riem(M') \leq 0 \). Then for
   every smooth map \( f_0:\ M \longrightarrow M'\subset B\subset \mathbb{R}^N \), the nonlinear heat equation
   \begin{cases}
...
uniformly bounded, a convergent subsequence \( \{f_{t_{n_i}}\} \) in \( C^k(M,
\mathbb{R}^N) \). By a diagonalisation argument, one can extract from any sequence \(
...
\). The homotopic conlusion follows by rescaling the deformation time between \( f_{t_n}
\) and \( f_{t_{n+1}} \) to \( \frac{1}{2^n} \).
#+END_proof
