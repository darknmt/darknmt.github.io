...
* General machinery by Morse and Palais.
...
interest in the case \( \alpha \) close to \( 1 \), let us also suppose that all \( \alpha
...
\begin{align*}
  p: \ C^0(M,N) &\longrightarrow N\\
       s	&\longmapsto	 f(z_0)
\end{align*}
then \( p \) is a fiber bundle and therefore a /Serre fibration/. In fact fix \( q_0\in N
...
1. The sequence \( s_\alpha \) and \( s \) extend to \( C^1(\bar\Omega) \) and have finite norm \( \max_\Omega |s| +
   ...
   C^1(\bar\Omega) \). If no other information is given, we will suppose this notion of convergence.
...
\), hence \( \overline{D(1/2)} \), we than obtain a contradiction using the uniqueness of
weak limit.
...
ref:lem:uni-loc-finite-cover (a consequence of the [[file:sobolev-riemannian.org][comparison of the Riemannian
manifold \( M \) with spaceforms]]), \( h \) can be chosen independently of \( m \) as \(
2^{-m} \to 0 \).
...
2. Since \( E(s) \leq \|s\|_{W^{1,2}}^2 \leq \liminf_{\alpha\to 1} \|s_\alpha\|^2 \leq
...
E_\alpha(s_\alpha) \) in \( (1+\delta,B) \). We claim that \( \{s_\alpha\} \) cannot converges
...
- If \( \{s_\alpha\} \) converge in \( C^1(M) \) to a certain \( \tilde s \), then as
...
Suppose that were not the case, then there exist \( C,\delta >0 \) such that \(
...
- \( s_\alpha \) converges weakly to \( s_* \) in \( W^{1,2}(D(R), \text{Euclidean}) \),
- \( E(s_\alpha) < B \)
