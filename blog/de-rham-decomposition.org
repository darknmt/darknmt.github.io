...
We first explain the appearance of /Euclidean/ components $E,E'$. They are product of trivial
...
We first note that the uniqueness stated in Proposition [[prop:uniqueness]] come from the uniqueness of
...
#+BEGIN_remark
One may note that a similar result is _not true_ for general representations: one can prove the
...
#+BEGIN_remark
1. The holonomy representation $H\subset SO(V)$ on a fiber $V=T_xM$ satisfies the property *(H)*: if
...
2. An example of representation that do satisfies this property *(H)* is the group $G = \{\pm
...
We prove the following lemma, which is evidently more general than Lemma [[lem:uniqueness-fiber]].
...
* Application of uniqueness lemma: decomposition for Kahler manifold
Now let apply the de Rham decomposition to a complete Riemannian manifold $M$ with holonomy $SU(n)
\subset SO(2n)$ (called a /Kahler manifold/). There exists on a fixed fiber $T_xM$ an automorphism
