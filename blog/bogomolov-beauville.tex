...
\ref{orgd79d9ad}). We note \(\Gamma := \{\phi = (\phi_1,\phi_2)\in \pi_1(X),\ \phi_2 = Id_M\}\) and
...
\label{sec:org16e5954}
To obtain a translation of Theorem \ref{org56c9e8f} in a context of complex manifolds (without any
...
By Theorem \ref{org56c9e8f}, \(\tilde X\) is compact by Lemma \ref{orgd79d9ad} a finite covering of
...
Theorem \ref{org56c9e8f}, there exists a finite cover \(X' = T\times \prod_i V_i\times \prod_j X_j\)
...
Theorem \ref{org655afbf} allows us to check if a manifold \(X\) is special unitary by looking at the
...
For the last point, one uses the same arguments as Remark \ref{org6a14232}.
...
Theorem \ref{org56c9e8f}, \(X = \prod_{j=1}^m X_j\) where \(X_j\) are irreducible compact Kähler
...
\begin{exampl}[Hyperkähler manifold]
One can notice a trivial example: Every special unitary manifold of 2 complex dimensions is
hyperkähler because \(SU(2)\) is isomorphic to \(Sp(1)\). There are not many known examples of
hyperkähler manifolds, except from sporadic ones in dimension 6 and 10, only two
infinite families are known.
\end{exampl}
\subsection{Decomposition for complex manifold with vanishing Chern class}
\label{sec:orgf475eb4}

Theorem \ref{org56c9e8f} can be translated to a decomposition for complex manifold in the following
way:
...
\end{enumerate}
\end{theorem}
The theorem follows directly from Theorem \ref{org56c9e8f}, the only point that needs proof is the
...
from uniqueness of Theorem \ref{org56c9e8f}.
...
\(Aut(Y)\), which is discrete by Lemma \ref{orgd79d9ad}. Therefore \(u_2(t,\cdot)= u_2(0,\cdot)\) independent of \(t\).
\end{proof}
