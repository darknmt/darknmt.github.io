...
In the language that we will use to solve [[./elliptic-parabolic.org][linear equation]], the diagram
eqref:fig:unique-interpol-pair is equivalent to the square
\[
 \xymatrix{
...
1. The condition \( X_0+X_1 \hookrightarrow Y_0+Y_1 \) being of closed image is redundant
   ...
   interpolation of /certain/ Sobolev spaces on manifolds. In general context, one can also check that this condition holds for the
   maps \( \iota_{k,p} \) and \( \iota_{l,q} \) in Definition ref:def:sobolev-space of
   Sobolev spaces using the fact that the \( \iota \) admit left-inverse given by \(
...
projection \( \pi:\ \bigoplus W^{k,p}(\mathbb{R}^n) \longrightarrow W^{k,p}(M) \) with \(
...
is manifolds with boundary and \( \mathcal{A} \) is union of connected components of \( \partial M
\) the boundary of \( M \). These spaces contain \( W^{k,p}(M) \) "functions" who vanish
on \( A \). The appearance of the parameter \( \mathcal{A} \) is because we will later
take \( M = M'\times [0,T] \) where \( M' \) is a manifold without boundary where we want
to solve heat equation, and the natural \( \mathcal{A} \) would be \( \partial M \times
\{0\} \). We also want that when the new definition coincides with the case of no boundary
...
1. The *Sobolev spaces* \( W^{k,p}(M/ \mathcal{A}) \) where \( A \) is a connected
   ...
   Euclidean space containing \( U_i \), that is either \( \mathbb{R}^{n+1} \) when \( \mathcal{A}_i=\emptyset \) or \(
...
boundary. Its proof follows from 2 remarks: On reduces the lemma, by interpolation
inequality, to the case \( k \) is a multiple of \( \sigma \) and use the criteria in
Theorem ref:thm:equiv-norm-Sobolev. Then in this case, use the boundedness of derivative
...
being a diffeomorphism mapping \( \mathcal{A}_U\subset \partial U \) to \( \mathcal{A}_V \subset \partial V \)
bijectively and of bounded derivatives. Let \( 0\leq \psi \leq 1 \) be a smooth function compactly supported in \( U
...
The Sobolev spaces on half-plan \( X\times Y^+ \) therefore has to have the following
properties: they have to be Banach spaces, satisfying Interpolation theorem
ref:thm:interp-sobolev and having \( \mathcal{S}(M) \) as a dense subspace. We will sketch
...
all their derivatives, at \( X\times \{0\} \). Idem for \( \mathcal{S}(X\times Y^-) \) and
...
is equivalent to the fact that \( C_+ \) admits a retraction, noted \( E_+ \) since it is
...
\begin{align*}
  E_+: \mathcal{S}(X\times Y^+) &\longrightarrow \mathcal{S}(X\times Y) \\
  	f 		     	  &\longmapsto	  \left( (x,y) \longmapsto \begin{cases}
								       ...
								       \end{cases} \right)
\end{align*}
This pushes the difficult part to the construction of \( \varphi \), which is resolved by
the following lemma.
...
\end{equation}
idem for \( \mathcal{S}(X\times Y^+/0 \) and \( \mathcal{S}(X\times Y^-) \) and the
...
\end{equation}
where the first coupling is on \( \mathcal{S}(X\times Y) \times \mathcal{S}(X\times Y) \)
and the second is on \( \mathcal{S}(X\times Y^+)\times \mathcal{S}(X\times Y^+/0) \).

#+ATTR_HTML: :id rem:coupling-S
#+BEGIN_remark
#+LATEX: \label{rem:coupling-S}
1. The two couplings satisfy \( \langle D^\alpha u, v \rangle = (-1)^{|\alpha|} \langle u,
   D^\alpha v \rangle \).
2. The second coupling give two natural identifications
   \[
    \mathcal{S}(X\times Y^+/0) \hookrightarrow \mathcal{S}^*(X\times Y^+),\quad
   \mathcal{S}(X\times Y^+) \hookrightarrow \mathcal{S}^*(X\times Y^+/0)
   \]
   while the first one gives \( \mathcal{S}(X\times Y) \hookrightarrow
   \mathcal{S}^*(X\times Y)\).
3. eqref:eq:Eplus-Rplus shows that \( E_+ \) and \( R_+ \) are adjoint, strictly speaking
   ...
   \]
#+END_remark
Also, one has \( \langle C_- f, g \rangle = \langle f, Z_- g \rangle \), hence similarly,
one has
\[
     \xymatrix{
...
so that the Sobolev spaces form an intermediate row in diagram, since the center cell \(
...
2. The topology of \( W^{k,p}(X\times Y) \) being finer than the induced of weak-* topology from \(
   ...
   \restr{Z_+^*}{W^{k,p}}\subset W^{k,p}(X\times Y) \), which is also the image by \( C_-^* \) of
   \(W^{k,p}(X\times Y^-/0) \), is a closed subspace of the Banach space \(
   W^{k,p}(X\times Y) \). Therefore \( W^{k,p}(X\times Y^-/0) \) and \( W^{k,p}(X\times
   Y^+) \) are Banach spaces.
...
#+BEGIN_remark
By dualizing the diagram eqref:eq:ses-S-W-Sstar and using the fact that the dual space of
...
38-42]] in the proof of Vanishing trace theorem ref:thm:vanishing-trace. The reason of
this paragraph is that I do not want to consider these results as technical details, and I
...
i.e. \(E^*_-\) and \( R^*_+ \) because the way we define extension operators \( E_+ \) and
\( R_- \) for smooth functions that satisfy
...
E_+ \) respectively) from half-plan to the whole plan. By the continuity of \( R_+^* \) in the second row of eqref:eq:ses-S-W-Sstar when \( k=0 \), one see that
...
measure 0). The strategy is to take a sequence \( f_n\in \mathcal{S}(X\times Y^+) \)
...
From diagram eqref:eq:ses-S-W-Sstar and its opposite version (with all \( + \) sign
replaced by \( - \) and vice versa), there is a natural inclusion \(\iota:\ W^{k,p}(X\times
Y^+/0) \) to \( W^{k,p}(X\times Y^+) \), by, for smooth functions, first extending by zero, then cut-off
\[
 \xymatrix{
...
If \( \rho=\sigma_i \) then \( \sigma=1 \), take \( k=0 \) then the Theorem
...
\end{cases}
\]
hence by Interpolation inequality ref:thm:interp-ineq-op, one obtains for all \( 0\leq
...
\) by the quotient \( W^{k,p} \)-norm instead of any \( W^{l,p} \)-norm. On one hand, we have completeness for free. On the
other hand, since \( B:\ W^{k,p}(X\times Y^+) \longrightarrow
L^p(X) \) factorizes to any \( W^{l,p}(X) \), the quotient \( W^{k,p} \)-norm is stronger
than any \( W^{l,p} \)-norm.
#+END_remark
...
that \( B f_+ = B f_- \) in \( L^p(X) \), one define \( f\in L^p(X\times Y) \) such that \(f=
...
that the data \( g_i \) correspond to a unique element \( g\in W^{l,p}(\mathcal{B})
