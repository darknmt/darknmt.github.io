...
def expandByFieldNonEV(pos, theta, arrAngleField, arrHistory, N, sqrPoint=[0,0], sqrSide=1, time=0):
    #Where am I
    a = sqrSide*1.0/N
    [i, j] = NearestCellIndexFromPos(pos, N, sqrPoint, sqrSide)
    if (i >N-1) or (i<1) or (j>N-1) or (j <1):
        return None
    i =int(i)
    j = int(j)
    if (arrHistory[i,j]==-1) or arrHistory[i,j]>(time-SPACETOLERANCE):
        arrHistory[i,j] = time
    else:
        return None #What else can I do ... To be completed
...
arrHistory = np.zeros([N,N])
for i in range(N):
    for j in range(N):
        if arrHasValue[i,j]:
            arrHistory[i,j] = -1
        else:
            arrHistory[i,j] = -3 #already passed
...
while dequeue:
    time += 1
    (pos, theta) = dequeue.popleft()
    res = expandByFieldNonEV(pos, theta, arrAngleField, arrHistory, N, sqrPoint=[-2,-2], sqrSide=4,time=time)
    print res
    if res != None:
        dequeue.append(res)
