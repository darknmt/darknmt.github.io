...
#Point = (pos, theta, ID)
def expandByFieldNonEV(point, arrAngleField, arrHistory, arrHasValue, N, sqrPoint=[0,0], sqrSide=1):
    #Where am I
    pos, theta, ID = point
    a = sqrSide*1.0/N
    ...
    if(arrHistory[i][j] != []) and arrHistory[i][j][0][-1] != ID:
        ...
        for p in arrHistory[i][j]:
            ...
            if tmp<dist:
                localpos = p[0]
                dist = tmp
        lineUp(pos,localpos, c='blue', lw=1)
        return None

    checkCell = []
    if (i<N-1) and arrHistory[i+1][j] != [] and arrHistory[i+1][j][0][-1] != ID:
        checkCell = arrHistory[i+1][j]
...
while dequeue:
    ...
    res = expandByFieldNonEV(currentPoint, arrAngleField, arrHistory, arrHasValue, N, sqrPoint=[-2,-2], sqrSide=4)
    print res
    if res != None:
        dequeue.append(res)
