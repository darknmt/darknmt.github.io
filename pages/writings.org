...
* A suspicious formula
  ...
  Euclidean space \( R^q \). The tricky detail, (which, in my oppinion, is a bit oversimplified in
  cite:eells_two_1995), involves using at the same time 2 metric structres on a same space:
...
- For \(Q\) in N, \pi(Q)\in M\) denotes the (geodesic) projection of Q on M and
  ...
  \( \mathbb{R}^q \), one can write \(p(Q) = Q - \pi(Q)\).
- Denote by \( w^a, w^b,w^c,\dots \) the coordinate in Euclidean fram of \( \mathbb{R}^q
...
For any vector \( u,v\in T_Q M' \) the vector \(\rho^c_{ab}u^a u^b \) is perpendicular to \(T_{Q}M'\)
#+END_lemma

I believe that the first half of eqref:eq:double (hence also the Lemma) is incorrect. 

#+BEGIN_remark
In fact, all applications of the Lemma and eqref:eq:double in the section 7 of
cite:eells_harmonic_1964 concern the second half, in order to claim that any vector
\( \frac{\partial^2\pi^c}{\partial w^a \partial w^b}U^b  V^a \tilde e_c \) is perpendicular to \( TM'
\), if the vectors \( U,V \) tangent to \( M \).
#+END_remark
...
pr_{TM'}(\Gamma''^c_{ab}\tilde e_c w^a_\alpha w^b_\beta) \), meaning that
\(
...
\]
where \( \nabla \) is Levi-Civita connection corresponding to Euclidean metric, and \(
\tilde e_b \) is the \( b^th \) vector field corresponding to the canonical coordinate of
